...
class PosQcLocalLregStrCH240nmCalc(PosQcCalc):
    short_time = 20     # 2回測定の測定時間差の最大値。単位min。
    ignore_list_name = 'ignore_list.pkl'

    # Excel関係
    p_trend_format = Path(os.path.dirname(__file__) + '/../../excel_template/posQC_trend/Local Lreg STR CH240nm Trend format.xlsx')
    excel_file_stem = 'Local_Lreg_STR_240nm'
    excel_start_row = 6
    excel_start_column = 1

    # 測定ファイルの検索用
    search_word = '*.lreg'
    search_meas_file = '**/*.lreg'
    ...
    def save_result_csv(self, p_csv=False):
        ...
        # 自分でカラム名つけてね。
        df = pd.DataFrame(list_result, columns=['Date', 'local_str_ch240nm_x', 'local_str_ch240nm_y'])
        if p_csv:
            df.to_csv(p_csv)
        else:
            return df
...
if __name__ == '__main__':
    p = Path(os.path.dirname(__file__) + '/../../../data/IPRO8/local_lreg_str_ch240nm/')
    p_from = Path('//172.26.31.68/d/measurement/result/QC/Local_CH240')
    p_mid = Path(os.path.dirname(__file__) + '/../../../mid_data/IPRO8/local_lreg_str_ch240nm/')
    p_save = Path(os.path.dirname(__file__) + '/../../../result/IPRO8/local_lreg_str_ch240nm/')
    p_csv = Path(os.path.dirname(__file__) + '/../../../result/IPRO8/Local Lreg STR CH240nm Trend.csv')
    p_trend = Path(os.path.dirname(__file__) + '/../../../result/IPRO8/excel_trend/Local Lreg STR CH240nm Trend.xlsx')
    t = PosQcLocalLregStrCH240nmCalc(p, p_mid, p_from,)
