...
from cd_qc.cd_qc_magnification_error import CdQcMagnificationError


class CdQcMagnificationErrorCalc(CdQcCalc):
    ignore_list_name = 'ignore_list.pkl'

    # Excel関係
    p_trend_format = Path(os.path.dirname(__file__) + '/../../excel_template/cdQC_trend/Magnification Error Trend.xlsx')
    ...
    def _read_pickle(self, list_meas_set):
        # Dynamic Shortを読み込む
        c = CdQcMagnificationError(list_meas_set[1], list_meas_set[2])
        return c
    ...
    def save_result_csv(self, p_csv=False):
        ...
        for r in list_instance:
            # 結果インスタンスから引き出す数値
            list_result_tmp = [r.meas_start, r.mag_error_hor, r.mag_error_ver,]
            list_result.append(list_result_tmp)
        # 自分でカラム名つけてね。
        df = pd.DataFrame(list_result, columns=['Date', 'magnification_error_hor', 'magnification_error_ver'
                                                ])
        if p_csv:
...
if __name__ == '__main__':
    p = Path(os.path.dirname(__file__) + '/../../../data/E3640/magnification_error/')
    p_from = Path('//172.26.31.68/d/measurement/result/QC/Screen_Linearity')
    p_mid = Path(os.path.dirname(__file__) + '/../../../mid_data/E3640/magnification_error/')
    p_save = Path(os.path.dirname(__file__) + '/../../../result/E3640/magnification_error/')
    p_csv = Path(os.path.dirname(__file__) + '/../../../result/E3640/Magnification Error.csv')
    p_trend = Path(os.path.dirname(__file__) + '/../../../result/excel/E3640/magnification error.xlsx')
    t = CdQcMagnificationErrorCalc(p, p_mid, search_plate=True)
